...
def check_export_rate_limit(project_id: str) -> None:
    """Rate-limit /api/export (cycles 160, 220, 280, 340, 400, 460, 520, 580, 640 — friendly 429 message)."""
    now = time.time()
...
class RequestIdMiddleware(BaseHTTPMiddleware):
    """Attach X-Request-ID to every response (cycles 148, 208, 268, 328, 388, 448, 508, 568, 628)."""
...
def openapi_description():
    """OpenAPI-style schema describing platform export endpoints (cycles 232, 292, 352, 412, 472, 532, 592, 652)."""
    return {
        "openapi": "3.0.3",
        "info": {
...
async def export_project_preflight():
    """CORS preflight for browser cross-origin export requests (cycles 124, 244, 304, 364, 424, 484, 544, 604, 664)."""
