...
def check_export_rate_limit(project_id: str) -> None:
    """Rate-limit /api/export (cycle 160, cycle 220 — friendly 429 message)."""
    now = time.time()
...
@app.get("/api/health")
def health():
    """Platform health including node-count (cycles 76, 196, 256)."""
    return {
        "status": "ok",
...
async def export_project_preflight():
    """CORS preflight for browser cross-origin export requests (cycles 124, 244)."""
