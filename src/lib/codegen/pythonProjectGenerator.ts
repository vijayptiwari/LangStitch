...
import { formatNodeModule, nodeRuntimeFunctionName } from './nodeModuleCodegen'
  ...
  return JSON.stringify(
    {
      ...
      ...(hasEvalDataset
        ? {
            // cycles 75, 135, 195, 255, 315, 375, 411, 435, 495, 555, 615, 675, 735 — eval-dataset metadata in langsmith.json export
            'eval-dataset': {
              enabled: true,
              dataset_name: ev?.datasetName ?? '',
              dataset_id: ev?.datasetId ?? '',
...
function generateNodeModules(
  ...
  for (const canvas of Object.values(canvasByGraph)) {
    for (const node of canvas.nodes) {
      ...
      seen.add(fn)
      imports.push(`from .${slugify(node.id)} import ${fn}`)

      files[modulePath(doc, 'nodes', `${slugify(node.id)}.py`)] = formatNodeModule(doc, node, pkg)
    }
  }
...
export function generatePythonProject(
  ...
  const monolith = generatePythonCode(doc, nodes, edges, canvases, { externalNodeModules: true })
  const files: Record<string, string> = {}
...
Generated by [LangStitch](https://github.com/LangStitch/langtailor) on the
